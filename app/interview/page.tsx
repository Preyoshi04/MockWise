...
export default function InterviewPage() {
  ...
  // --- REFINED CAMERA LOGIC ---
  const toggleCamera = async () => {
    // 1. Show warning if toggling during a live session
    if (isCalling) {
      toast.warning("Camera State Changed", {
        description: "Consistency is important for AI analysis. Try to keep the camera state stable.",
        duration: 4000,
      });
    }
  ...
  useEffect(() => {
    ...
    vapi.on("call-end", () => {
      ...
      // Cleanup: Stop camera tracks when the call ends
      if (stream) {
        stream.getTracks().forEach(track => track.stop());
      }
    ...
    return () => { 
      vapi.stop(); 
      // Cleanup tracks on unmount using the current stream reference
      if (stream) {
        stream.getTracks().forEach(track => track.stop());
      }
    };
    // Crucial: Removed 'stream' from dependencies to prevent re-initializing Vapi on camera toggle
  }, [router]); 
        ...
        <div className="flex flex-col md:flex-row items-center gap-12">
          ...
          <div className={`relative group transition-all duration-700 ease-in-out ${cameraActive ? 'opacity-100 scale-100 w-64 h-48 md:w-80 md:h-60' : 'opacity-0 scale-95 w-0 h-0 overflow-hidden'}`}>
            <video 
              ...
              playsInline 
              muted 
              className="w-full h-full object-cover rounded-[2rem] border-2 border-zinc-800 bg-zinc-900 shadow-2xl transition-all duration-500"
            />
